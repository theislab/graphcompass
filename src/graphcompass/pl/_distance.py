...
def convert_dataframe(df_ct, samples):
    ...
    df = pd.DataFrame(data=np.full((num_samples, num_samples), fill_value=np.nan), columns=samples, index=samples)
...
def compare_conditions(
    ...
    figsize: tuple[float, float] | None = (20, 10),
    palette="Greys",
    dpi: int | None = 300,
    ...
    if plot_groups_separately:
        ...
        if add_ncells_and_density_plots:
            ...
            fig, axes = plt.subplots(
                num_contrasts, 3, figsize=(width, height * num_contrasts), dpi=dpi
            ...
            for i, contrast in enumerate(df_for_plot["contrast"].unique()):
                ...
                palette = {
                    ct: get_bar_color(mean_vals[ct], mean_vals.max(), mean_vals.min(), "Greens")
                    for ct in mean_vals.index
                }
                ...
                # Plot 2
                sns.barplot(data=contrast_properties, y=y, x="ncells", hue="condition", palette="tab20", ax=axes[i, 1])
                axes[i, 1].set_ylabel("")
                ...
                sns_plot = sns.barplot(
                    data=contrast_properties, y=y, x="density", hue="condition", palette="tab20", ax=axes[i, 2]
                )
        ...
        if add_ncells_and_density_plots:
            ...
            result_ncells = (
                properties.groupby(["condition", "cell_type"])["ncells"].agg(["median", "var"]).reset_index()
            )
            result_ncells.columns = ["condition", "cell_type", "median", "variance"]
            result_density = (
                properties.groupby(["condition", "cell_type"])["density"].agg(["median", "var"]).reset_index()
            )
        ...
        if add_ncells_and_density_plots:
            n_subplots = 3
            fig, axes = plt.subplots(n_subplots, 1, figsize=(width, height * n_subplots), dpi=dpi)
            ax = axes[0]
        else:
            n_subplots = 1
            fig, ax = plt.subplots(n_subplots, 1, figsize=(width, height * n_subplots), dpi=dpi)
        ...
        legend_elements = [
            plt.Line2D([0], [0], marker="o", color="w", markerfacecolor="k", markersize=np.sqrt(size), label=label)
            for size, label in zip(sizes, labels)
        ]  # Only create labeled elements
